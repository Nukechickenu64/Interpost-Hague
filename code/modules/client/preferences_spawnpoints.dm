...
/datum/spawnpoint/cryo/after_join(mob/living/carbon/human/victim, obj/machinery/computer/cryopod/control_computer)
	...
	for(var/obj/machinery/cryopod/C in A)
		...
		if(!C.occupant)
			...
			victim.Sleeping(7)
			victim.resting = 0
			victim.give_cryo_advice()
			victim.give_cryo_effect()
			victim.add_event("cryo", /datum/happiness_event/cryo)
			victim.add_cryo_filter_effect()
			addtimer(CALLBACK(victim, /mob/proc/remove_cryo_filter_effect), 40 SECONDS, TIMER_UNIQUE|TIMER_NO_HASH_WAIT)
			return
...
/datum/spawnpoint/cryocaptain/after_join(mob/living/carbon/human/victim, obj/machinery/computer/cryopod/control_computer)
	...
	for(var/obj/machinery/cryopod/C in A)
		if(!C.occupant)
			...
			victim.resting = 0
			victim.give_cryo_advice()
			victim.give_cryo_captain_effect()
			victim.add_event("cryo", /datum/happiness_event/cryo)
			addtimer(CALLBACK(C, /obj/machinery/cryopod/proc/go_out_forced), 25 SECONDS, TIMER_UNIQUE|TIMER_NO_HASH_WAIT)
			victim.add_cryo_filter_effect()
			addtimer(CALLBACK(victim, /mob/proc/remove_cryo_filter_effect), 40 SECONDS, TIMER_UNIQUE|TIMER_NO_HASH_WAIT)
			return
	for(var/obj/machinery/light/L in A)
