...
/obj/item/weapon/folder/attack_self(mob/user as mob)
	...
	user << browse(page, "window=folder")
	onclose(user, "folder")
	add_fingerprint(usr)
	return
		...
		if(href_list["remove"])
			...
			if(P && (P.loc == src) && istype(P))
				if(!(istype(usr, /mob/living/carbon/human) || isghost(usr) || istype(usr, /mob/living/silicon)))
					var/body = "[stars(P.info)][P.stamps]"
					var/page = ui_build_styled_html(P.name, body)
					usr << browse(page, "window=[P.name]")
					onclose(usr, "[P.name]")
				else
					var/body2 = "[P.info][P.stamps]"
					var/page2 = ui_build_styled_html(P.name, body2)
