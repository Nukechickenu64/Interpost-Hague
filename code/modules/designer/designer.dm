...
/datum/designer_system/proc/newDesign(var/datum/designer_unit/unit)
	...
	if(designs.len > 1)
		var/datum/designer_unit/prev = designs[designs.len - 1]
		unit.id = prev.id + 1
	else
		unit.id = 1
...
/datum/designer_system/proc/delete_designs()
	...
	for (var/datum/designer_unit/design in GLOB.designer_system.designs)
		if (design.creator_ckey == lowertext(ckey))
			design.delete_icon()
			count++
	to_chat(usr, "[count] designs made by [ckey] have been purged, only their associated atoms were left behind.")
