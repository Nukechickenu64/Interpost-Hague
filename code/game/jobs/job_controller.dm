 ...
 **/
	...
	proc/EquipRank(mob/living/carbon/human/H, rank, joined_late = 0)
		...
		if(H.mind.assigned_role == "Captain")
			spawn(20)
				to_chat("<B>Enough ruminating on recent events, I'm the captain of this shithole and I've got a job to do and by [H.religion] I'll do it.")
				spawn(50)
					to_chat(H, "<B>.......<B> ")
					spawn(50)
						to_chat(H, "<B>Damn these pods, wasted ten minutes shaking off the cryo-sickness and I still feel awful.<B> ")
						spawn(50)
							to_chat(H, "<B>.......<B> ")
							spawn(50)
								to_chat(H, "<B>I should gather the officers for a meeting. <font color = 'purple'>Tetracorp</font>'s bound to have sent us some new job to do...<B> ")
		else
			spawn(20)
				...
				spawn(20)
					to_chat(H, "<B>.......<B> ")
					spawn(20)
						to_chat(H, "<B>It's....it's freezing...must be in a cryo pod.")
						spawn(20)
							to_chat(H, "<B>.......<B> ")
							spawn(20)
								to_chat(H, "<B>Gotta focus...where am I?")
								spawn(20)
									to_chat(H, "<B>.......<B> ")
									spawn(60)
										to_chat(H, "<B>Right, right...I'm [job.total_positions == 1 ? "the" : "a"] [alt_title ? alt_title : rank] working for <font color = 'purple'>Tetracorp</font> on one of their ''state of the art'' research outposts. ")
										spawn(20)
											to_chat(H, "<B>.......<B>")
											spawn(100)
												if(job.supervisors)
